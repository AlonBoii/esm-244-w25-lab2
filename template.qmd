...
lm_1 <- lm(f1, data = penguins_clean)
lm_2 <- lm(f2, data = penguins_clean)
lm_3 <- lm(f3, data = penguins_clean)
...
AIC(lm_1,lm_2,lm_3) # the lowest AIC is the best, lm_2
```
...
for loop :
    apply the model to each training set (fold)
    make predictions on the teset set with fitted training model
close sloop:
...
penguins_fold<-penguins_clean |> 
  mutate(group = sample(fold_vec, size = n(), replace = F)) # we use False because we don't want to resample
  

#check to make sure the fold groups are balanced

table(penguins_fold$group) # to make sure that all groups are balanced (as close as possible)
```
...
train_df <- penguins_fold |>
  filter(group != 1) # != if it's not equal to 1, give me everything else
...
rmse_predict_test<-predict_test |> 
  summarize(rmse_lm1=calc_rmse(mass, model1),
            rmse_lm2=calc_rmse(mass, model2),
            rmse_lm3=calc_rmse(mass, model3)) # lowest RMSE should be the chosen one
...
for( i in 1:folds){
  ...
  rmse_vec[i]<-rmse_predict_test$rmse_lm1
}
...
Model 2 is the best and what should we use
